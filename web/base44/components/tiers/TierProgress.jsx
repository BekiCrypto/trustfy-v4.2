    ...
    return (
      <Card className="bg-gradient-to-br from-cyan-500/10 to-blue-500/10 border-cyan-500/30 p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-white">{t('tiers.platinumTier')}</h3>
    ...
    <Card className="bg-slate-900/50 border-slate-700/50 p-6">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <p className="text-sm text-slate-400">{t('tiers.progress')} {nextConfig.name}</p>
          ...
          <div key={idx} className="space-y-1">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className={req.current >= req.required ? 'text-emerald-400' : 'text-slate-300'}>
                {req.label === 'Trading Volume' ? '$' : ''}{req.current.toLocaleString()} / {req.label === 'Trading Volume' ? '$' : ''}{req.required.toLocaleString()}
              </span>
            </div>
            <Progress 
              value={req.progress} 
