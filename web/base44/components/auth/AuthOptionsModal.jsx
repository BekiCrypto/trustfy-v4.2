...
export default function AuthOptionsModal({ open, onOpenChange }) {
  ...
  const handleWalletConnect = async () => {
    try {
      // Directly open Web3Modal for wallet selection
      // This opens the wallet connection UI (MetaMask, WalletConnect QR, etc.)
      await openWeb3Modal();
      // Modal is now open - user will select wallet
      // Don't set isAuthenticating here - let them connect first
    } catch (error) {
        ...
        <div className="grid md:grid-cols-2 gap-6 py-6">
          ...
          <Card className="relative bg-gradient-to-br from-blue-500/10 to-purple-500/10 border-blue-500/30 p-6 hover:border-blue-500/50 transition-all">
            ...
            <p className="text-xs text-slate-400 text-center mt-3 leading-relaxed">
              Use WalletConnect to login with MetaMask, Trust Wallet, Coinbase Wallet, Binance Wallet, OKX Wallet, and other Web3 wallets.
            </p>
          </Card>
          ...
          <Card className="bg-slate-800/50 border-slate-700 p-6">
            ...
            <p className="text-xs text-amber-400 text-center mt-4">
              Note: You'll need to connect a wallet before your first trade
            </p>
          </Card>
        </div>
