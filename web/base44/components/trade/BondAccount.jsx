...
export default function BondAccount({ chain = 'BSC', tokens = ['USDT', 'USDC', 'BNB'] }) {
  ...
  const handleWithdraw = async (token) => {
    ...
    if (parseFloat(amount) > available) {
      toast.error('Amount exceeds available credits');
      return;
    }
    ...
    try {
      ...
      toast.error(error.message || 'Failed to withdraw bond credits');
    } finally {
      setWithdrawing(null);
    }
  };
                ...
                <div className="space-y-2 pt-3 border-t border-slate-700">
                  ...
                  {!canWithdraw && hasCredits && (
                    <p className="text-xs text-amber-400 mt-2">
                      ⚠️ Minimum {WITHDRAW_THRESHOLD} {token} required to withdraw
                    </p>
                  )}
                </div>
              </div>
