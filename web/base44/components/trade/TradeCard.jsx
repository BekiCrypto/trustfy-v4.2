...
export default function TradeCard({ trade, index = 0 }) {
  ...
  const getActionBadge = () => {
    ...
    if (trade.status === 'pending' && isSeller) {
      return { text: 'Fund Escrow', color: 'amber' };
    }
    if (trade.status === 'funded' && isBuyer) {
      return { text: 'Lock Bond & Pay', color: 'blue' };
    }
    if (trade.status === 'in_progress' && isSeller) {
      return { text: 'Release Funds', color: 'emerald' };
    }
    return null;
  };
          ...
          <div className="flex-1 space-y-3">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <Link to={createPageUrl(`TradeDetails?id=${trade.id}`)}>
                <button className="flex items-center gap-1 text-xs text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded-full border border-blue-500/30 hover:bg-blue-500/20 transition-all">
