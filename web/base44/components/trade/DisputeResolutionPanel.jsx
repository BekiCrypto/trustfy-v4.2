  ...
  const getRulingDisplay = () => {
    if (!isResolved || ruling === 'pending') {
      return {
        ...
        description: 'Waiting for arbitrator decision'
      };
    }
    ...
    if (ruling === 'favor_buyer') {
      return {
        ...
        label: 'Buyer Wins',
        description: 'Buyer receives crypto + both bonds refunded'
      };
    }
    ...
    if (ruling === 'favor_seller') {
      return {
        ...
        label: 'Seller Wins',
        description: 'Seller gets both bonds, buyer receives crypto'
      };
    }
        ...
        {bondAmount && (
          <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/30">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-sm font-semibold text-red-400">Financial Stakes</h4>
            </div>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-slate-400">Seller Bond at Risk:</span>
                <span className="text-white font-semibold">{bondAmount} {trade.token_symbol}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Buyer Bond at Risk:</span>
                <span className="text-white font-semibold">{bondAmount} {trade.token_symbol}</span>
              </div>
              ...
              {ruling === 'favor_buyer' && (
                <>
                  ...
                  <div className="flex justify-between text-red-300">
                    ...
                    <span className="font-bold">{bondAmount} {trade.token_symbol} bond → Platform</span>
                  </div>
                </>
              )}
              {ruling === 'favor_seller' && (
                <>
                  ...
                  <div className="flex justify-between text-red-300">
                    ...
                    <span className="font-bold">{bondAmount} {trade.token_symbol} bond</span>
                  </div>
                </>
              )}
            </div>
        ...
        {!isResolved && (
          <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/30">
            <p className="text-amber-300 text-xs">
              ...
              The arbitrator will review evidence from both parties. The winner receives their bond back plus the loser's bond (which goes to the platform as dispute fee). The crypto amount always goes to the buyer.
            </p>
          </div>
        )}
      </div>
