...
export default function MatchedOffersModal({ open, onOpenChange, offer, allOffers = [] }) {
  ...
  const createTrade = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      toast.success('Trade matched successfully!', {
  ...
  const createTradeLegacy = useMutation({
    mutationFn: async (matchedOffer) => {
      ...
      const trade = await base44.entities.Trade.create({
        ...
        terms: `Trade created from marketplace matching. Rate: ${fiatCurrency} ${pricePerUnit} per ${offer.token_symbol}. Seller escrows ${escrowAmount.toFixed(2)} ${offer.token_symbol} (includes all fees). Buyer pays ${fiatCurrency} ${totalFiatAmount.toFixed(2)} (includes buyer fee).`
      });
      ...
      <DialogContent className="bg-slate-900 border-slate-700 text-white max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <Alert className="bg-purple-500/10 border-purple-500/30">
          ...
          <AlertDescription className="text-purple-300 text-xs">
            <strong>Dispute Bond Required:</strong> Accepting creates a trade with symmetric bonds. 
            {offer.offer_type === 'sell' ? ' You\'ll lock your bond when funding escrow.' : ' You\'ll lock your bond when confirming fiat payment.'}
            {' '}Bond refunded on completion or if you win any dispute.
          </AlertDescription>
        </Alert>
                  ...
                  <div className="p-2 rounded-lg bg-blue-500/10 border border-blue-500/30">
                    <p className="text-xs text-blue-300">
                      ...
                      {offer.offer_type === 'sell' 
                        ? 'Next Step: Fund escrow + lock your bond to activate trade'
                        : 'Next Step: Wait for seller to fund, then pay & lock your bond'}
                    </p>
                  </div>
                  
                  <Button
                    onClick={() => createTrade.mutate(match)}
                  ...
                  >
                    {createTrade.isPending ? (
                      ...
                      <>
                        ...
                        {offer.offer_type === 'sell' ? 'Accept - Go to Escrow' : 'Accept - Go to Trade'}
                      </>
                    )}
                  </Button>
                </div>
