...
export default function ArbitratorDisputeCard({ dispute, resolved = false }) {
  ...
  const resolveDispute = useMutation({
    mutationFn: async ({ disputeId, rulingDecision, rulingNotes }) => {
      ...
      try {
        ...
        throw new Error('Failed to resolve dispute on blockchain: ' + error.message);
      }
      ...
      if (trade) {
        ...
        await Promise.all([
          createNotification({
            ...
            message: `The dispute has been resolved. Ruling: ${rulingDecision.replace('_', ' ')}`,
    ...
    onSuccess: () => {
      ...
      toast.success('Dispute resolved on-chain and database updated');
      setExpanded(false);
    },
    onError: (error) => {
  ...
  const handleResolve = (decision) => {
    ...
    const confirmMsg = aiAnalysis 
      ? `Resolve in favor of ${decision === 'favor_seller' ? 'seller' : 'buyer'}? ${decision === aiAnalysis.recommended_ruling ? '(Matches AI recommendation)' : '⚠️ (Overrides AI recommendation)'}`
      : `Resolve in favor of ${decision === 'favor_seller' ? 'seller' : 'buyer'}?`;
  ...
  const handleAIAnalysisComplete = (analysis) => {
    ...
    if (analysis.confidence_score >= 70 && !notes.trim()) {
      setNotes(`Based on AI analysis (${analysis.confidence_score}% confidence):\n\n${analysis.reasoning}\n\nKey findings:\n${analysis.key_evidence?.slice(0, 3).map(e => `- ${e}`).join('\n') || ''}`);
    }
  };
            ...
            {resolved && dispute.ruling && (
              <Badge className={
                ...
                {dispute.ruling === 'favor_seller' ? t('cards.arbitrator.sellerWon') : 
                 dispute.ruling === 'favor_buyer' ? t('cards.arbitrator.buyerWon') : 'Split'}
              </Badge>
            )}
          </div>
        ...
        {trade && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="p-3 rounded-lg bg-slate-800/30">
              ...
              <p className="text-white font-semibold">Tier {dispute.escalation_level}</p>
            </div>
            <div className="p-3 rounded-lg bg-slate-800/30">
              <p className="text-xs text-slate-500 mb-1">{t('cards.arbitrator.fee')}</p>
              <p className="text-white font-semibold">{dispute.dispute_fee}%</p>
            </div>
          </div>
        )}
      </div>
              ...
              <AlertDescription className="text-purple-300 text-sm">
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span>Case Age:</span>
                    <span className="font-semibold">{ageHours.toFixed(0)} hours</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <Badge className={
                      ...
                      {urgencyScore > 75 ? 'Critical' : urgencyScore > 50 ? 'High' : 'Normal'}
                    </Badge>
                  </div>
            ...
            <div className="pt-6 border-t border-slate-700/50 space-y-4">
              ...
              <Link to={createPageUrl(`TradeDetails?id=${trade?.id}`)}>
                <Button variant="outline" className="w-full border-slate-600">
                  <ArrowLeftRight className="w-4 h-4 mr-2" />
                  {t('cards.arbitrator.viewFullTrade')}
                </Button>
              </Link>
            </div>
            </RoleGuard>
