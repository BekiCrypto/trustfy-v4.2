...
const escalationConfig = {
  1: { 
    ...
    bg: 'bg-violet-500/10',
    description: 'Automated analysis of evidence and trade history'
  },
  2: { 
    ...
    bg: 'bg-orange-500/10',
    description: 'Review by certified platform arbitrator'
  },
  3: { 
...
export default function DisputeDetails() {
  ...
  const runAIAnalysis = async () => {
    ...
    try {
      ...
      const prompt = `You are an AI arbitrator analyzing a trade dispute. Provide a fair, unbiased ruling based on the evidence.

DISPUTE DETAILS:
- Reason: ${dispute.reason}
- Description: ${dispute.description}
- Trade Amount: $${trade.amount} ${trade.token_symbol}
- Chain: ${trade.chain}
...
Return as JSON with this structure:
{
  ...
  "key_points": ["point 1", "point 2", "point 3"]
      ...
      toast.error('AI analysis failed, escalating to human arbitration');
  ...
  const handleEscalate = async () => {
    if (dispute.escalation_level < 3) {
      ...
      // Notify both parties
      const notifData = NotificationTemplates.disputeEscalated(disputeId, newLevel);
      await createNotification({
        userAddress: trade.seller_address,
        ...notifData,
        metadata: { dispute_id: disputeId, trade_id: dispute.trade_id }
      });
      await createNotification({
  ...
  const handleAcceptAIRuling = async () => {
    ...
    try {
      ...
      toast.info('Resolving dispute on blockchain...');
      ...
      // Notify both parties
      const notifData = NotificationTemplates.disputeResolved(disputeId, aiAnalysis.suggested_ruling);
      await createNotification({
        userAddress: trade.seller_address,
        ...notifData,
        metadata: { dispute_id: disputeId, trade_id: dispute.trade_id }
      ...
      toast.success('AI ruling accepted and dispute resolved on-chain');
    } catch (error) {
      toast.error('Failed to resolve on blockchain: ' + error.message);
    }
  };
  ...
  const handleContestAIRuling = () => {
    ...
    toast.info('Dispute escalated to Tier 2 arbitration');
  };
...
Return detailed JSON:
{
  ...
  "risk_assessment": "assessment of any risks or concerns with this ruling"
      ...
      toast.error('Tier 2 arbitration failed, manual review required');
    } finally {
      setIsAnalyzing(false);
    }
  };
  ...
  const handleAcceptTier2Ruling = async () => {
    ...
    try {
      ...
      toast.success('Tier 2 ruling accepted and dispute resolved on-chain');
    } catch (error) {
      toast.error('Failed to resolve on blockchain: ' + error.message);
    }
  };
  ...
  const handleEscalateToDAO = () => {
    ...
    toast.info('Dispute escalated to DAO governance voting');
  };
  ...
  const handleSubmitRuling = async () => {
    if (!ruling) {
      toast.error('Please select a ruling');
      return;
    }
    
    if (!rulingReason.trim()) {
      toast.error('Please provide ruling notes');
      return;
    }
    ...
    try {
      ...
      toast.info('Blockchain transaction submitted', {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-8">
        <div className="max-w-2xl mx-auto text-center py-20">
          ...
          <p className="text-slate-400 mb-6">The dispute you&rsquo;re looking for doesn&rsquo;t exist.</p>
        ...
        >
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-2xl font-bold text-white">Appeal Details</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
                ...
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 text-sm text-slate-500">
                      ...
                      Opened {dispute.created_date && format(new Date(dispute.created_date), "MMM d, yyyy 'at' HH:mm")}
                    </div>
                  </div>
                </div>
                  ...
                  {dispute.arbitrator_address && (
                    <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700/50">
                      <div className="flex items-center gap-2 text-slate-400 mb-3">
                        ...
                        <span className="text-sm font-medium">Arbitrator</span>
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-400">
                          Analyzing trade data, chat history, and evidence...
                        </p>
                      </div>
                    </div>
                  </Card>
                ...
                {isAnalyzing ? (
                  <Card className="bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50 backdrop-blur-xl p-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-lg font-semibold text-white">Tier 2 Arbitration in Progress</h3>
                        <p className="text-sm text-slate-400">
                          Conducting comprehensive analysis of all dispute data, Tier 1 ruling, and evidence...
                        </p>
                      </div>
                    </div>
                  </Card>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {dispute.evidence_urls.map((url, index) => (
                      ...
                      >
                        ...
                        <p className="text-sm text-slate-300">Evidence {index + 1}</p>
                      </a>
                    ))}
                  </div>
                ) : (
            ...
            {trade && (
              ...
              >
                <Card className="bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50 backdrop-blur-xl p-6">
                  <h3 className="text-lg font-semibold text-white mb-4">Related Trade</h3>
          ...
          >
            ...
            <Card className="bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50 backdrop-blur-xl p-6">
              ...
              {dispute.status !== 'resolved' && dispute.escalation_level < 3 && (
                ...
                >
                  Escalate to {escalationConfig[dispute.escalation_level + 1]?.label}
                </Button>
              )}
            </Card>
              ...
              <Card className="bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50 backdrop-blur-xl p-6">
                ...
                <div className="p-3 rounded-lg bg-purple-500/10 border border-purple-500/30 mb-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-purple-400 text-sm font-semibold">Bond Distribution</span>
                  </div>
                  <p className="text-slate-300 text-xs">
                    Winner gets their bond back. Loser&rsquo;s bond goes to platform as dispute fee. 
                    Crypto amount always goes to buyer.
                  </p>
                </div>
                ...
                <RadioGroup value={ruling} onValueChange={setRuling} className="space-y-3">
                  <div className="flex flex-col space-y-2 p-3 rounded-lg bg-slate-800/50 border border-slate-700/50">
                    ...
                    <p className="text-xs text-slate-400 ml-7">
                      Seller gets both bonds • Buyer gets crypto • Buyer bond → Platform
                    </p>
                  </div>
                  <div className="flex flex-col space-y-2 p-3 rounded-lg bg-slate-800/50 border border-slate-700/50">
                    <div className="flex items-center space-x-3">
                    ...
                    <p className="text-xs text-slate-400 ml-7">
                      Buyer gets crypto + both bonds • Seller bond → Platform
                    </p>
                  </div>
                </RadioGroup>
                
                <Textarea
                  placeholder="Provide reasoning for your ruling..."
                  value={rulingReason}
            ...
            {dispute.status === 'resolved' && (
              <Card className="bg-gradient-to-br from-emerald-500/10 to-green-500/10 border-emerald-500/30 p-6">
                <div className="text-center">
                  ...
                  <h3 className="text-lg font-semibold text-white mb-2">Dispute Resolved</h3>
                  ...
                  {bondAmount && (
                    <div className="p-4 rounded-lg bg-slate-800/50 border border-slate-700/50 mb-4 text-left">
                      <p className="text-sm font-semibold text-white mb-3">Payout Distribution:</p>
                      <div className="space-y-2 text-xs">
                        {dispute.ruling === 'favor_buyer' && (
                          <>
                            <div className="flex justify-between text-emerald-300">
                              <span>→ Buyer receives:</span>
                              <span className="font-bold">{trade.amount} {trade.token_symbol} + {bondAmount} {trade.token_symbol} bond</span>
                            </div>
                            <div className="flex justify-between text-red-300">
                              <span>→ Seller forfeits:</span>
                              <span className="font-bold">{bondAmount} {trade.token_symbol} → Platform</span>
                            </div>
                          </>
                        )}
                        {dispute.ruling === 'favor_seller' && (
                          <>
                            <div className="flex justify-between text-blue-300">
                              ...
                              <span className="font-bold">{(parseFloat(bondAmount) * 2).toFixed(4)} {trade.token_symbol} (both bonds)</span>
                            </div>
                            <div className="flex justify-between text-emerald-300">
                              <span>→ Buyer receives:</span>
                              <span className="font-bold">{trade.amount} {trade.token_symbol}</span>
                            </div>
                            <div className="flex justify-between text-red-300">
                              <span>→ Buyer forfeits:</span>
                              <span className="font-bold">{bondAmount} {trade.token_symbol}</span>
                            </div>
                          </>
                        )}
                      </div>
                  ...
                  {dispute.resolved_at && (
                    <p className="text-slate-500 text-xs mt-3">
                      Resolved on {format(new Date(dispute.resolved_at), "MMM d, yyyy 'at' HH:mm")}
                    </p>
                  )}
                </div>
              </Card>
