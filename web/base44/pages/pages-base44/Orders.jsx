      ...
      <div className="relative max-w-7xl mx-auto space-y-6">
        ...
        <Alert className="bg-gradient-to-r from-purple-500/10 to-blue-500/10 border-purple-500/30">
          ...
          <AlertDescription className="text-purple-300 text-sm">
            <strong>{t('bondCredits.smartBondSystem')}:</strong> {t('ordersPage.smartBondsNotice')}
          </AlertDescription>
        </Alert>
        ...
        <Tabs value={statusFilter} onValueChange={setStatusFilter}>
          <TabsList className="bg-slate-800/50 border border-slate-700 p-1 flex-wrap h-auto">
            ...
            <TabsTrigger value="unpaid" className="data-[state=active]:bg-slate-700 text-slate-300">
              {t('ordersPage.awaitingBonds')} ({statusCounts.unpaid})
            </TabsTrigger>
            <TabsTrigger value="paid" className="data-[state=active]:bg-slate-700 text-slate-300">
              {t('ordersPage.bondsLocked')} ({statusCounts.paid})
            </TabsTrigger>
        ...
        {!isLoading && filteredTrades.length > 0 && (
          <div className="text-sm text-slate-400">
            {t('ordersPage.showing')} {filteredTrades.length} {t('ordersPage.of')} {trades.length} {t('ordersPage.orders')}
          </div>
        )}
          ...
          {isLoading ? (
            ...
            >
              ...
              <h3 className="text-lg font-semibold text-slate-300 mb-2">
                {searchQuery ? t('orders.noOrdersFound') : t('orders.noOrdersYet')}
              </h3>
