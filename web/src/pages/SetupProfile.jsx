    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-400">{t('setupProfile.checking')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center">
        <Card className="bg-slate-900 border-slate-700 p-12 text-center max-w-md">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">{t('setupProfile.foundTitle')}</h2>
          <p className="text-slate-400">{t('setupProfile.redirecting')}</p>
        </Card>
      </div>
    );
  }
  ...
  if (status === 'success') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center">
        <Card className="bg-slate-900 border-slate-700 p-12 text-center max-w-md">
          <CheckCircle className="w-16 h-16 text-green-500 mx-auto mb-4" />
          <h2 className="text-2xl font-bold text-white mb-2">{t('setupProfile.createdTitle')}</h2>
          <p className="text-slate-400 mb-4">{t('setupProfile.createdBody')}</p>
          <p className="text-sm text-slate-500">
            {t('setupProfile.roleLabel')} {profile?.platform_role || t('setupProfile.roleDefault')}
          </p>
        </Card>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center p-8">
      <Card className="bg-slate-900 border-slate-700 p-12 text-center max-w-md">
        ...
        <h2 className="text-2xl font-bold text-white mb-2">{t('setupProfile.welcomeTitle')}</h2>
        <p className="text-slate-400 mb-6">
          {t('setupProfile.welcomeBody')}
        </p>
        <p className="text-sm text-slate-500 mb-6">
          {t('setupProfile.emailLabel')} {user?.email}
        </p>

        {status === 'error' && (
          <p className="text-red-400 text-sm mb-4">
            {t('setupProfile.error')}
          </p>
        )}
