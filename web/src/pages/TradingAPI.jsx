...
export default function TradingAPI() {
  ...
  const generateAPIKey = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(t('tradingBots.toast.apiKeyGeneratedTitle'), {
        description: t('tradingBots.toast.apiKeyGeneratedDesc')
      });
    },
    onError: () => {
      toast.error(t('tradingBots.toast.apiKeyGenerateFailed'));
    }
  });
  ...
  const deleteAPIKey = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['current-profile'] });
      toast.success(t('tradingBots.toast.apiKeyRevoked'));
    }
  });
  ...
  const analyzeStrategy = async () => {
    if (!strategyInput.trim()) {
      toast.error(t('tradingBots.toast.strategyRequired'));
      return;
    }
...
Return a JSON response with this exact structure:
{
  ...
  "improvements": ["array of strategy improvements"]
      ...
      toast.success(t('tradingBots.toast.strategyAnalyzed'));
    } catch (error) {
      toast.error(t('tradingBots.toast.strategyAnalyzeFailed'));
      console.error(error);
    } finally {
  ...
  const detectArbitrage = async () => {
    ...
    try {
      ...
      if (sortedOpps.length > 0) {
        ...
        toast.success(t('tradingBots.toast.arbitrageFoundTitle', { count: sortedOpps.length }), {
          description: t('tradingBots.toast.arbitrageFoundDesc', {
            roi: sortedOpps[0].netProfitPercent,
            token: sortedOpps[0].token
          })
        });
      } else {
        toast.info(t('tradingBots.toast.noArbitrageTitle'), {
          description: t('tradingBots.toast.noArbitrageDesc')
        });
      }
    } catch (error) {
      toast.error(t('tradingBots.toast.arbitrageScanFailed'));
      console.error(error);
    } finally {
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4 md:p-8">
        ...
        <div className="relative max-w-3xl mx-auto">
          ...
          >
            ...
            <h1 className="text-3xl font-bold text-white mb-2">{t('tradingApiPage.title')}</h1>
            <p className="text-slate-400">{t('tradingApiPage.primeSubtitle')}</p>
          </motion.div>
          ...
          <Card className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 border-purple-500/30 p-8">
            ...
            <div className="space-y-3 mb-8">
              <div className="flex items-center gap-2 text-slate-300">
                ...
                <span>{t('tradingApiPage.benefits.restAccess')}</span>
              </div>
              <div className="flex items-center gap-2 text-slate-300">
                <CheckCircle2 className="w-5 h-5 text-emerald-400" />
                <span>{t('tradingApiPage.benefits.aiAnalysis')}</span>
              </div>
              <div className="flex items-center gap-2 text-slate-300">
                <CheckCircle2 className="w-5 h-5 text-emerald-400" />
                <span>{t('tradingApiPage.benefits.arbitrageScanner')}</span>
              </div>
              <div className="flex items-center gap-2 text-slate-300">
                <CheckCircle2 className="w-5 h-5 text-emerald-400" />
                <span>{t('tradingApiPage.benefits.unlimitedKeys')}</span>
              </div>
              <div className="flex items-center gap-2 text-slate-300">
                <CheckCircle2 className="w-5 h-5 text-emerald-400" />
              ...
              >
                ...
                <Card className="bg-slate-900/50 border-slate-700/50 p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="p-4 rounded-lg bg-slate-800/50">
                      <p className="text-xs text-slate-500 mb-1">{t('tradingApiPage.riskLevel')}</p>
                    ...
                    <div className="p-4 rounded-lg bg-slate-800/50">
                      <p className="text-xs text-slate-500 mb-1">{t('tradingApiPage.profitPotential')}</p>
                  ...
                  <div className="space-y-3">
                    <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/30">
                      <p className="text-sm font-medium text-blue-400 mb-2">{t('tradingApiPage.parsedStrategy')}</p>
                      <div className="text-xs text-slate-300 space-y-1">
                        <p><span className="text-slate-500">{t('tradingApiPage.actionLabel')}</span> {strategyAnalysis.parsed_strategy.action}</p>
                        <p><span className="text-slate-500">{t('tradingApiPage.tokenLabel')}</span> {strategyAnalysis.parsed_strategy.token}</p>
                        <p><span className="text-slate-500">{t('tradingApiPage.conditionLabel')}</span> {strategyAnalysis.parsed_strategy.condition}</p>
                        <p><span className="text-slate-500">{t('tradingApiPage.triggerLabel')}</span> {strategyAnalysis.parsed_strategy.trigger_percentage}%</p>
                      </div>
                    </div>
                  </div>
                </Card>
                ...
                <Card className="bg-slate-900/50 border-slate-700/50 p-6">
                  <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                    ...
                    {t('tradingApiPage.riskAssessmentTitle')}
                  </h3>
                  
                  <div className="space-y-4">
                    <div>
                      <p className="text-sm font-medium text-slate-300 mb-2">{t('tradingApiPage.identifiedRisks')}</p>
                    ...
                    <div>
                      <p className="text-sm font-medium text-slate-300 mb-2">{t('tradingApiPage.mitigationSuggestions')}</p>
                      <ul className="space-y-2">
                ...
                <Card className="bg-slate-900/50 border-slate-700/50 p-6">
                  <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                    ...
                    {t('tradingApiPage.implementationParams')}
                  </h3>
                  
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 rounded-lg bg-slate-800/50">
                      <p className="text-xs text-slate-500 mb-1">{t('tradingApiPage.entryPrice')}</p>
                      <p className="text-sm text-white font-mono">{strategyAnalysis.implementation.entry_price}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-800/50">
                      <p className="text-xs text-slate-500 mb-1">{t('tradingApiPage.exitPrice')}</p>
                      <p className="text-sm text-white font-mono">{strategyAnalysis.implementation.exit_price}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-800/50">
                      <p className="text-xs text-slate-500 mb-1">{t('tradingApiPage.stopLoss')}</p>
                      <p className="text-sm text-red-400 font-mono">{strategyAnalysis.implementation.stop_loss}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-800/50">
                      <p className="text-xs text-slate-500 mb-1">{t('tradingApiPage.positionSize')}</p>
                      <p className="text-sm text-white font-mono">{strategyAnalysis.implementation.position_size}</p>
                    </div>
                  </div>

                  <div className="mt-4">
                    <p className="text-sm font-medium text-slate-300 mb-2">{t('tradingApiPage.suggestedImprovements')}</p>
                    <ul className="space-y-2">
                    ...
                    {opportunityHistory.slice(0, 5).map((entry, idx) => (
                      <div key={idx} className="flex items-center justify-between p-3 rounded-lg bg-slate-800/30 border border-slate-700/50 text-sm">
                        ...
                        <div className="flex items-center gap-4">
                          <div className="text-slate-400">
                            <span className="text-white font-medium">{entry.count}</span> {t('tradingBots.opps')}
                          </div>
                          <div className="text-slate-400">
                            {t('tradingBots.best')}: <span className="text-emerald-400 font-medium">{entry.bestRoi}%</span> {t('tradingBots.on')} {entry.bestToken}
                          </div>
                          <div className="text-slate-400">
                            {t('tradingBots.potential')}: <span className="text-cyan-400 font-medium">${entry.totalPotentialProfit}</span>
                          </div>
                        </div>
                      </div>
                    ))}
                    ...
                    <Card className="relative bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-emerald-500/30 p-6 hover:border-emerald-500/50 transition-all hover:shadow-xl hover:shadow-emerald-500/20 overflow-hidden group">
                      ...
                      <div className="relative flex items-start justify-between mb-6">
                            ...
                            <div className="flex items-center gap-3 text-xs">
                              ...
                              {parseFloat(opp.netProfitPercent) > 5 && (
                                <div className="flex items-center gap-1 text-amber-400 animate-pulse">
                                  <Flame className="w-3 h-3" />
                                  <span className="font-medium">{t('tradingApiPage.hotDeal')}</span>
                                </div>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-sm text-slate-400 font-medium">{t('tradingApiPage.netRoi')}</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        <div className="p-4 rounded-lg bg-slate-800/50 border border-slate-700 relative">
                          ...
                          <p className="text-xs text-slate-500 mb-2">{t('tradingApiPage.stepBuy')}</p>
                        ...
                        <div className="p-4 rounded-lg bg-emerald-900/20 border border-emerald-500/30 relative">
                          ...
                          <p className="text-xs text-emerald-400 mb-2">{t('tradingApiPage.stepSell')}</p>
                      ...
                      <div className="p-3 rounded-lg bg-slate-800/30 border border-slate-700/50 mb-4">
                        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                          <div className="flex justify-between">
                            <span className="text-slate-500">{t('tradingApiPage.priceDifference')}</span>
                            <span className="text-white font-mono">${opp.priceDiff.toFixed(4)}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-500">{t('tradingApiPage.grossProfit')}</span>
                            <span className="text-emerald-400 font-mono">+${opp.grossProfit}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-500">{t('tradingApiPage.maxVolume')}</span>
                            <span className="text-white font-mono">{opp.maxAmount.toFixed(2)} {opp.token}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-500">{t('tradingApiPage.estimatedCosts')}</span>
                      ...
                      <div className="flex gap-2">
                        <Button 
                          ...
                          onClick={() => toast.info(t('tradingBots.toast.autoExecutionSoon'))}
                        >
                          <Play className="w-4 h-4 mr-2" />
                          {t('tradingBots.executeArbitrage')}
                        </Button>
                        <Button 
                          variant="outline" 
                          className="border-slate-600"
                          onClick={() => {
                            const details = t('tradingBots.arbitrageDetails', {
                              token: opp.token,
                      ...
                      {parseFloat(opp.investment) > 1000 && (
                        <div className="mt-3 p-2 rounded-lg bg-amber-500/10 border border-amber-500/30 flex items-start gap-2">
                          ...
                          <p className="text-xs text-amber-400">
                            {t('tradingBots.highValueWarning')}
                          </p>
                        </div>
                      )}
                    </Card>
              ...
              {apiKeys.length > 0 && (
                <div className="grid grid-cols-3 gap-3 mb-6">
                  ...
                  <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700">
                    ...
                    <p className="text-2xl font-bold text-purple-400">{t('tradingApiPage.rateLimitCompact')}</p>
                  </div>
                </div>
              )}
            </Card>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="grid grid-cols-2 gap-3 pt-3 border-t border-slate-700/50">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs text-slate-400">{t('tradingApiPage.rateLimitLabel')}</span>
                              <span className="text-xs text-white font-medium">{t('tradingApiPage.rateLimitValue')}</span>
                            </div>
                          </div>
                        </div>
                      </Card>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    <div className="border border-slate-700 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="px-2 py-1 bg-emerald-500/20 text-emerald-400 text-xs font-mono rounded">{t('common.httpMethods.get')}</span>
                        <code className="text-sm text-white">{t('tradingApiPage.endpoints.offers')}</code>
                      </div>
                    ...
                    <div className="border border-slate-700 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="px-2 py-1 bg-blue-500/20 text-blue-400 text-xs font-mono rounded">{t('common.httpMethods.post')}</span>
                        <code className="text-sm text-white">{t('tradingApiPage.endpoints.offers')}</code>
                      </div>
                    ...
                    <div className="border border-slate-700 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="px-2 py-1 bg-emerald-500/20 text-emerald-400 text-xs font-mono rounded">{t('common.httpMethods.get')}</span>
                        <code className="text-sm text-white">{t('tradingApiPage.endpoints.trades')}</code>
                      </div>
                    ...
                    <div className="border border-slate-700 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="px-2 py-1 bg-purple-500/20 text-purple-400 text-xs font-mono rounded">{t('common.httpMethods.put')}</span>
                        <code className="text-sm text-white">{t('tradingApiPage.endpoints.tradesId')}</code>
                      </div>
