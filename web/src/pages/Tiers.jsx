      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        {isLoading ? (
          <Card className="bg-slate-900/50 border-slate-700/50 p-6 mb-8">
            <div className="flex items-center justify-center py-8">
              ...
              <span className="text-slate-400">{t('tiersPage.loadingProgress')}</span>
            </div>
          </Card>
        ) : profile ? (
          <motion.div
            ...
            <Card className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 border-purple-500/30 p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-xl font-bold text-white">
                  {t('tiersPage.pathTo', { tier: nextTier.charAt(0).toUpperCase() + nextTier.slice(1) })}
                </h3>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-slate-300">{t('tiersPage.reputationScoreLabel')}</span>
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-slate-300">{t('tiersPage.successfulTradesLabel')}</span>
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-slate-300">{t('tiersPage.tradingVolumeLabel')}</span>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-slate-900/50 border border-slate-700/50 mb-6">
            <TabsTrigger value="overview">{t('tiersPage.tabOverview')}</TabsTrigger>
            <TabsTrigger value="comparison">{t('tiersPage.tabComparison')}</TabsTrigger>
            <TabsTrigger value="requirements">{t('tiersPage.tabRequirements')}</TabsTrigger>
          </TabsList>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-slate-800/50 border-b border-slate-700">
                    <tr>
                      <th className="text-left p-4 text-slate-400 text-sm font-medium">{t('tiersPage.benefit')}</th>
                  ...
                  <tbody>
                    <tr className="border-b border-slate-800">
                      <td className="p-4 text-white font-medium">{t('tiersPage.escrowPriority')}</td>
                      {Object.keys(TIER_BENEFITS).map(tier => (
                        <td key={tier} className="text-center p-4">
                          <Badge className="bg-emerald-500/20 text-emerald-400">
                            {TIER_BENEFITS[tier].makerFeeDiscount > 0 ? t('tiersPage.included') : t('tiersPage.standardLabel')}
                          </Badge>
                        </td>
                      ))}
                    </tr>
                    <tr className="border-b border-slate-800">
                      <td className="p-4 text-white font-medium">{t('tiersPage.verificationBoost')}</td>
                      {Object.keys(TIER_BENEFITS).map(tier => (
                        <td key={tier} className="text-center p-4">
                          <Badge className="bg-blue-500/20 text-blue-400">
                            {TIER_BENEFITS[tier].takerFeeDiscount > 0 ? t('tiersPage.included') : t('tiersPage.standardLabel')}
                          </Badge>
                        </td>
                      ))}
                    </tr>
                    <tr className="border-b border-slate-800">
                      <td className="p-4 text-white font-medium">{t('tiersPage.protectionSupport')}</td>
                      {Object.keys(TIER_BENEFITS).map(tier => (
                        <td key={tier} className="text-center p-4">
                          <Badge className="bg-purple-500/20 text-purple-400">
                            {TIER_BENEFITS[tier].insuranceDiscount > 0 ? t('tiersPage.available') : t('tiersPage.standardLabel')}
                          </Badge>
                        </td>
                      ))}
                    </tr>
                    <tr className="border-b border-slate-800">
                      <td className="p-4 text-white font-medium">{t('tiersPage.prioritySupport')}</td>
                      {Object.keys(TIER_BENEFITS).map(tier => (
                        <td key={tier} className="text-center p-4">
                    ...
                    <tr className="border-b border-slate-800">
                      <td className="p-4 text-white font-medium">{t('tiersPage.featuredListing')}</td>
                      {Object.keys(TIER_BENEFITS).map(tier => (
                        <td key={tier} className="text-center p-4">
                    ...
                    <tr className="border-b border-slate-800">
                      <td className="p-4 text-white font-medium">{t('tiersPage.apiAccess')}</td>
                      {Object.keys(TIER_BENEFITS).map(tier => (
                        <td key={tier} className="text-center p-4">
                    ...
                    <tr className="border-b border-slate-800">
                      <td className="p-4 text-white font-medium">{t('tiersPage.higherTradeLimits')}</td>
                      {Object.keys(TIER_BENEFITS).map(tier => (
                        <td key={tier} className="text-center p-4">
            ...
            <div className="space-y-6">
              <Alert className="bg-blue-500/10 border-blue-500/30">
                ...
                <AlertDescription className="text-blue-300 text-sm">
                  <strong>{t('tiersPage.howToAdvanceLabel')}</strong> {t('tiersPage.howToAdvanceText')}
                </AlertDescription>
              </Alert>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <div>
                        <h4 className="text-sm font-semibold text-slate-300 mb-3">{t('tiersPage.requirementsTitle')}</h4>
                        <div className="space-y-2">
                          <div className="flex items-center justify-between p-2 rounded bg-slate-800/50">
                            <span className="text-slate-400 text-sm">{t('tiersPage.reputationScoreLabel')}</span>
                            <span className="text-white font-medium">{reqs.score}+</span>
                          </div>
                          <div className="flex items-center justify-between p-2 rounded bg-slate-800/50">
                            <span className="text-slate-400 text-sm">{t('tiersPage.successfulTradesLabel')}</span>
                            <span className="text-white font-medium">{reqs.trades}+</span>
                          </div>
                          <div className="flex items-center justify-between p-2 rounded bg-slate-800/50">
                            <span className="text-slate-400 text-sm">{t('tiersPage.tradingVolumeLabel')}</span>
                      ...
                      <div>
                        <h4 className="text-sm font-semibold text-slate-300 mb-3">{t('tiersPage.keyBenefitsTitle')}</h4>
                        <div className="space-y-2">
                          <div className="flex items-center gap-2 text-sm">
                            <Zap className="w-4 h-4 text-emerald-400" />
                            <span className="text-slate-300">{t('tiersPage.escrowPriority')}</span>
                          </div>
                          <div className="flex items-center gap-2 text-sm">
                            <Zap className="w-4 h-4 text-blue-400" />
                            <span className="text-slate-300">{t('tiersPage.verificationBoost')}</span>
                          </div>
                          {benefits.prioritySupport && (
                            <div className="flex items-center gap-2 text-sm">
                              <Shield className="w-4 h-4 text-purple-400" />
                              <span className="text-slate-300">{t('tiersPage.prioritySupport')}</span>
                            </div>
                          )}
                          {benefits.featuredListing && (
                            <div className="flex items-center gap-2 text-sm">
                              <Star className="w-4 h-4 text-amber-400" />
                              <span className="text-slate-300">{t('tiersPage.featuredListings')}</span>
                            </div>
                          )}
                          {benefits.apiAccess && (
                            <div className="flex items-center gap-2 text-sm">
                              <Award className="w-4 h-4 text-cyan-400" />
                              <span className="text-slate-300">{t('tiersPage.apiAccess')}</span>
                            </div>
                          )}
                        </div>
                      </div>
