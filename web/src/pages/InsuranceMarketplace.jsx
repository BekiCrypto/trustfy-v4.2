    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4 md:p-8">
      ...
      <div className="relative max-w-7xl mx-auto space-y-6">
        ...
        >
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-white tracking-tight flex items-center gap-3">
              ...
              {t('insuranceMarketplace.title')}
            </h1>
            <p className="text-slate-400 mt-1">
              {t('insuranceMarketplace.subtitle')}
            </p>
          </div>
          ...
          <Link to={createPageUrl('BecomeInsurer')}>
            <Button className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700">
              ...
              {t('insuranceMarketplace.becomeInsurer')}
            </Button>
          </Link>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatsCard
            title={t('insuranceMarketplace.stats.totalCapital')}
            value={`$${totalCapital.toLocaleString()}`}
            icon={TrendingUp}
            gradient="from-emerald-500/20 to-emerald-600/20"
            subtitle={t('insuranceMarketplace.stats.totalCapitalSubtitle')}
          />
          <StatsCard
            title={t('insuranceMarketplace.stats.activeProviders')}
            value={providers.filter(p => p.status === 'active').length}
            icon={Shield}
            gradient="from-blue-500/20 to-blue-600/20"
            subtitle={t('insuranceMarketplace.stats.totalProviders', { count: providers.length })}
          />
          <StatsCard
            title={t('insuranceMarketplace.stats.policiesWritten')}
            value={totalPolicies}
            icon={Star}
            gradient="from-purple-500/20 to-purple-600/20"
            subtitle={t('insuranceMarketplace.stats.policiesActive', { count: activePolicies })}
          />
          <StatsCard
            title={t('insuranceMarketplace.stats.claimsPaid')}
            value={`$${totalClaims.toLocaleString()}`}
            icon={TrendingUp}
            gradient="from-amber-500/20 to-amber-600/20"
            subtitle={t('insuranceMarketplace.stats.totalPayouts')}
          />
        </div>
        ...
        >
          <div className="relative flex-1">
            ...
            <Input
              placeholder={t('insuranceMarketplace.searchPlaceholder')}
              value={searchQuery}
          ...
          {isLoading ? (
            ...
            >
              ...
              <h3 className="text-lg font-semibold text-slate-300 mb-2">
                {searchQuery ? t('insuranceMarketplace.empty.searchTitle') : t('insuranceMarketplace.empty.noProvidersTitle')}
              </h3>
              <p className="text-slate-500 mb-4">
                {searchQuery 
                  ? t('insuranceMarketplace.empty.searchBody')
                  : t('insuranceMarketplace.empty.noProvidersBody')}
              </p>
              {!searchQuery && (
                <Link to={createPageUrl('BecomeInsurer')}>
