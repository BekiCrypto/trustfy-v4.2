...
export default function Appeal() {
  ...
  const helpItems = t('safety.disputes.when.items', { returnObjects: true });
      ...
      <div className="relative max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4">
            <p className="text-slate-400 text-sm mb-1">{t('arbitrator.totalDisputes')}</p>
            <p className="text-2xl font-bold text-white">{stats.totalDisputes}</p>
          </div>
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4">
            <p className="text-amber-400 text-sm mb-1">{t('adminStats.active')}</p>
            <p className="text-2xl font-bold text-white">{stats.activeDisputes}</p>
          </div>
          <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4">
            <p className="text-emerald-400 text-sm mb-1">{t('disputes.resolved')}</p>
            <p className="text-2xl font-bold text-white">{stats.resolvedDisputes}</p>
          </div>
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4">
            <p className="text-blue-400 text-sm mb-1">{t('arbitrator.averageTime')}</p>
        ...
        >
          ...
          <Tabs value={priorityFilter} onValueChange={setPriorityFilter} className="w-auto">
            <TabsList className="bg-slate-800/50 border border-slate-700">
              <TabsTrigger value="all" className="data-[state=active]:bg-slate-700">{t('common.all')}</TabsTrigger>
              <TabsTrigger value="critical" className="data-[state=active]:bg-red-700 text-xs">{t('settingsContent.criticalOnly')}</TabsTrigger>
              <TabsTrigger value="high" className="data-[state=active]:bg-orange-700 text-xs">{t('arbitrator.highPriority')}</TabsTrigger>
              <TabsTrigger value="medium" className="data-[state=active]:bg-yellow-700 text-xs">{t('arbitrator.mediumPriority')}</TabsTrigger>
            </TabsList>
          </Tabs>
        </motion.div>
        ...
        <Tabs value={statusFilter} onValueChange={setStatusFilter}>
          <TabsList className="bg-slate-800/50 border border-slate-700 p-1 flex-wrap h-auto">
            ...
            <TabsTrigger value="pending" className="data-[state=active]:bg-slate-700 text-slate-300">
              {t('arbitrator.pending')} ({statusCounts.pending})
            </TabsTrigger>
          ...
          {isLoading ? (
            ...
            >
              ...
              <p className="text-slate-500">
                {searchQuery 
                  ? t('disputes.adjustSearch')
                  : t('disputes.allEscrowsSmooth')}
              </p>
            </motion.div>
          ) : (
            <motion.div
        ...
        {!isLoading && filteredDisputes.length > 0 && (
          <div className="text-center text-sm text-slate-500 pt-4">
            {t('ordersPage.showing')} {filteredDisputes.length} {t('ordersPage.of')} {disputes.length} {t('disputes.title')}
          </div>
        )}
        ...
        {disputes.length === 0 && !isLoading && (
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-6">
            <h3 className="text-lg font-semibold text-blue-400 mb-2">{t('safety.disputes.title')}</h3>
