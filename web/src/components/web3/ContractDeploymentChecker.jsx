...
export default function ContractDeploymentChecker() {
  ...
  const checkContract = async () => {
    ...
    try {
      ...
      if (code === '0x') {
        setStatus('invalid');
        setContractInfo({ error: t('web3.contractChecker.noContract') });
        return;
      }
      ...
      try {
        ...
        setContractInfo({ 
          ...
          codeSize: code.length,
          verified: false,
          warning: t('web3.contractChecker.abiWarning')
        });
      }
    ...
    <div className="space-y-3">
      {status === 'checking' && (
        <div className="flex items-center gap-2 text-blue-400 text-sm">
          ...
          <span>{t('web3.contractChecker.checking')}</span>
        </div>
      )}
      ...
      {status === 'deployed' && (
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-emerald-400 text-sm">
            ...
            <span>{t('web3.contractChecker.live')}</span>
          </div>
      ...
      {status === 'not_deployed' && (
        <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/30">
          <div className="flex items-start gap-2 text-amber-400 text-sm mb-2">
            ...
            <span>{t('web3.contractChecker.notDeployedTitle')}</span>
          </div>
          <p className="text-xs text-slate-400">
            {t('web3.contractChecker.notDeployedDesc')}
          </p>
        </div>
      )}
      ...
      {status === 'invalid' && (
        <div className="flex items-start gap-2 text-red-400 text-sm">
          ...
          <div>
            <p>{t('web3.contractChecker.invalidAddress')}</p>
      ...
      {status === 'error' && (
        <div className="flex items-start gap-2 text-red-400 text-sm">
          <XCircle className="w-4 h-4 mt-0.5" />
          <div>
            <p>{t('web3.contractChecker.failedVerify')}</p>
            <p className="text-xs text-slate-500">{contractInfo?.error}</p>
          </div>
        </div>
      )}
      ...
      >
        {t('web3.contractChecker.recheck')}
      </Button>
    </div>
  );
}
