...
export default function OfferCard({ offer, index, onViewMatches, onAcceptOffer, showCancel = false, isMyOffer = false, matchCount }) {
  ...
  const stateMap = {
    open: { label: 'CREATED', color: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' },
    matched: { label: 'TAKEN', color: 'bg-blue-500/10 text-blue-400 border-blue-500/30' },
  ...
  const adState = stateMap[offer.status] || { label: offer.status?.toUpperCase() || 'UNKNOWN', color: 'bg-slate-700/30 text-slate-300 border-slate-600/50' };
  const isClickable = offer.status === 'open';
  ...
  const cancelOffer = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(t('marketplace.offerCard.toast.cancelSuccess'));
      setShowCancelDialog(false);
    },
    onError: (error) => {
      console.error('Cancel error:', error);
      toast.error(
        t('marketplace.offerCard.toast.cancelFailed', {
          message: error?.message || t('marketplace.offerCard.toast.cancelUnknown')
        })
      );
      setShowCancelDialog(false);
    }
    ...
    >
      <Card className="bg-gradient-to-br from-slate-900/80 to-slate-800/80 border-slate-700/50 backdrop-blur-xl p-5 hover:border-slate-600/50 transition-all duration-300">
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <Badge variant="outline" className={adState.color}>
              {adState.label}
            </Badge>
        ...
        <div className="flex gap-2">
          {isMyOffer ? (
            ...
            <Button
              ...
              size="sm"
              disabled={remaining <= 0 || !isClickable}
            >
              {remaining <= 0 ? (
                'Fully Filled'
              ) : !isClickable ? (
        ...
        {offer.status === 'cancelled' && (
          <p className="text-xs text-red-400 mt-3">
            Cancelled ads forfeit the AdBond.
          </p>
        )}
      </Card>
      ...
      <ConfirmDialog
        ...
        title="Cancel Ad?"
        description={`Are you sure you want to cancel this ${offer.offer_type} ad for ${offer.amount} ${offer.token_symbol}? This action cannot be undone.`}
        confirmText={cancelOffer.isPending ? "Cancelling..." : "Yes, Cancel Ad"}
