...
export default function CreateOfferModal({ open, onOpenChange }) {
  ...
  const createOffer = useMutation({
    ...
    onSuccess: (data) => {
      ...
      const offerTypeLabel = data.offer_type === 'buy' ? t('createOffer.buy') : t('createOffer.sell');
      toast.success(t('createOffer.toast.publishSuccessTitle'), {
        id: 'create-offer',
        description: t('createOffer.toast.publishSuccessDesc', {
          amount: data.amount,
    ...
    onError: (error) => {
      ...
      const errorMsg = error?.message || t('createOffer.toast.publishFailedTitle');
      toast.error(errorMsg, {
        id: 'create-offer',
        description: t('createOffer.toast.publishFailedDesc'),
        duration: 5000
      });
    }
  });
  ...
  React.useEffect(() => {
    const loadConfig = async () => {
      try {
        const cfg = await getTokenConfig(formData.chain, formData.token_symbol);
        setTokenConfig(cfg);
    ...
    loadConfig();
  }, [formData.chain, formData.token_symbol, getTokenConfig]);

  const adBondPreview =
    tokenConfig?.adBondBps && formData.amount
      ? ((Number(formData.amount) || 0) * tokenConfig.adBondBps) / 10_000
      : 0;
  ...
  const handleSubmit = (e) => {
    ...
    if (!validation.valid) {
      ...
      toast.error(t('createOffer.toast.validationFix'), {
    ...
    if (selectedPaymentMethods.length === 0) {
      toast.error(t('createOffer.toast.paymentMethodsRequired'), {
        description: t('createOffer.toast.paymentMethodsDesc')
      });
      return;
    }
    ...
    if (minTrade && maxTrade && minTrade > maxTrade) {
      toast.error(t('createOffer.toast.minTradeExceedsMax'));
      return;
    }

    if (maxTrade && maxTrade > amount) {
      toast.error(t('createOffer.toast.maxTradeExceedsAmount'));
      return;
    }
    
    // Clear errors and submit
    setValidationErrors({});
    toast.loading(t('createOffer.toast.publishing'), { id: 'create-offer' });
  ...
  const handleAddCustomPayment = () => {
    ...
    setCustomPaymentMethod('');
    setShowCustomPaymentInput(false);
    toast.success(t('createOffer.toast.customPaymentAdded'));
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-slate-900 border-slate-700/50 text-white max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              >
                ...
                <SelectContent className="bg-slate-800 border-slate-700">
                  <SelectItem value="BSC_TESTNET">BSC Testnet (97)</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label className="text-sm text-slate-300 mb-2 block">{t('createOffer.minEscrow')}</Label>
              <Input
                type="number"
                step="0.01"
            ...
            <div>
              <Label className="text-sm text-slate-300 mb-2 block">{t('createOffer.maxEscrow')}</Label>
              <Input
                type="number"
                step="0.01"
          ...
          <div className="space-y-2">
            ...
            {availablePaymentMethods.length > 0 ? (
              ...
              <div className="p-3 rounded-lg bg-slate-800/30 border border-slate-700/50 text-center">
                <p className="text-xs text-slate-400">{t('createOffer.noPaymentMethods')}</p>
              </div>
            )}
          ...
          <div className="p-3 rounded-lg bg-purple-500/10 border border-purple-500/30">
            <div className="flex items-start gap-2">
              ...
              <div className="text-xs text-slate-300">
                <p className="font-semibold text-purple-300 mb-1">AdBond & DisputeBond</p>
                <p>Creating an Ad locks the Maker’s AdBond. The Buyer’s DisputeBond is locked when the Ad is taken.</p>
                {adBondPreview > 0 && (
                  <p className="text-purple-300 mt-1">
                    AdBond preview: {adBondPreview.toFixed(4)} {formData.token_symbol}
                  </p>
                )}
                <p className="text-slate-400 mt-1">All bond outcomes are enforced on-chain per the escrow state.</p>
              </div>
            </div>
          </div>
          ...
          {configError && (
            <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/30 text-xs text-amber-300">
              Token config unavailable. Ad creation will remain disabled until on-chain config is reachable.
            </div>
          )}
          ...
          {tokenConfig && tokenConfig.enabled === false && (
            <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/30 text-xs text-red-300">
              This token is disabled by admin. You cannot create Ads for it.
            </div>
          )}
          ...
          <div className="pt-3 border-t border-slate-700/50">
            <Button
              type="submit"
              disabled={createOffer.isPending || tokenConfig?.enabled === false || !!configError}
