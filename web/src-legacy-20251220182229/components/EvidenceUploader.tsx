...
export const EvidenceUploader = ({ escrowId }: { escrowId: string }) => {
  ...
  return (
    <section className="card space-y-4 p-5">
      ...
      <form className="space-y-4 rounded-2xl border border-[color:var(--line)] bg-white/70 p-4" onSubmit={handleSubmit}>
        <div className="space-y-2">
          ...
          />
          {file && (
            <div className="flex items-center justify-between rounded-2xl border border-[color:var(--line)] bg-white px-3 py-2 text-xs text-[color:var(--muted)]">
              <span>{file.name}</span>
              <button
                type="button"
      ...
      <div className="space-y-3">
        ...
        {list?.map((item: EvidenceEntry) => (
          <article
            key={item.id}
            className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-[color:var(--line)] bg-white/70 px-3 py-2 text-xs text-[color:var(--muted)]"
          >
            <div>
              <p className="text-xs font-semibold text-slate-950">{item.description || "Evidence"}</p>
